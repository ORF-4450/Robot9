...
{
  static final String  	PROGRAM_NAME = "RAC9-19-05.06.19-01";

  // Motor CAN ID/PWM port assignments (1=left-front, 2=left-rear, 3=right-front, 4=right-rear)
  WPI_TalonSRX			LFWPI_TalonSRX, LRWPI_TalonSRX, RFWPI_TalonSRX, RRWPI_TalonSRX, LSlaveWPI_TalonSRX, RSlaveWPI_TalonSRX;
...
//        gyro.calibrate();
        ...
        // and IP address of system hosting the camera feed.
   		...
   		// Not used if we are feeding the DS from the Raspberry Pi by
  ...
  private void InitializeWPI_TalonSRXDrive()
  {
	  Util.consoleLog();

	  LFWPI_TalonSRX = new WPI_TalonSRX(1);
	  LRWPI_TalonSRX = new WPI_TalonSRX(2);
	  RFWPI_TalonSRX = new WPI_TalonSRX(3);
	  RRWPI_TalonSRX = new WPI_TalonSRX(4);
	  LSlaveWPI_TalonSRX = new WPI_TalonSRX(5);
	  RSlaveWPI_TalonSRX = new WPI_TalonSRX(6);
	  
	  robotDrive = new RobotDrive(LFWPI_TalonSRX, LRWPI_TalonSRX, RFWPI_TalonSRX, RRWPI_TalonSRX);

      // Initialize CAN Talons and write status to log so we can verify
      // all the talons are connected.
      Initialize_WPI_TalonSRX(LFWPI_TalonSRX);
      Initialize_WPI_TalonSRX(LRWPI_TalonSRX);
      Initialize_WPI_TalonSRX(RFWPI_TalonSRX);
      Initialize_WPI_TalonSRX(RRWPI_TalonSRX);
      Initialize_WPI_TalonSRX(LSlaveWPI_TalonSRX);
      Initialize_WPI_TalonSRX(RSlaveWPI_TalonSRX);
      
	  // Configure slave CAN Talons to follow the front L & R Talons.
	  LSlaveWPI_TalonSRX.set(ControlMode.Follower, LFWPI_TalonSRX.getDeviceID());
      RSlaveWPI_TalonSRX.set(ControlMode.Follower, RFWPI_TalonSRX.getDeviceID());
	  
	//   RFWPI_TalonSRX.set(ControlMode.Follower, RFWPI_TalonSRX_TalonSRX.getDeviceID());
	  
	//   LSlaveWPI_TalonSRX.changeControlMode(ControlMode.Follower);
	//   LSlaveWPI_TalonSRX.set(LFWPI_TalonSRX.getDeviceID());

    //   RSlaveWPI_TalonSRX.changeControlMode(ControlMode.Follower);
    //   RSlaveWPI_TalonSRX.set(RFWPI_TalonSRX.getDeviceID());
      
      // Turn on brake mode for CAN Talons.
      SetWPI_TalonSRXBrakeMode(true);
  }
  ...
  // Initialize and Log status indication from WPI_TalonSRX. If we see an exception
  // or a talon has low voltage value, it did not get recognized by the RR on start up.
  
  public void Initialize_WPI_TalonSRX(WPI_TalonSRX talon)
  {
  ...
  public void SetWPI_TalonSRXBrakeMode(boolean brakeMode)
  {
	...
	LFWPI_TalonSRX.setNeutralMode(newMode);
	LRWPI_TalonSRX.setNeutralMode(newMode);
	RFWPI_TalonSRX.setNeutralMode(newMode);
	RRWPI_TalonSRX.setNeutralMode(newMode);  
	LSlaveWPI_TalonSRX.setNeutralMode(newMode);
	RSlaveWPI_TalonSRX.setNeutralMode(newMode);  
  }
  
  // Set CAN Talon voltage ramp rate. Rate is volts/sec and can be 2-12v.
  
  public void SetWPI_TalonSRXRampRate(double rate)
  {
	  Util.consoleLog("%f", rate);
	  
	  LFWPI_TalonSRX.configOpenloopRamp(rate, 0);
	  LRWPI_TalonSRX.configOpenloopRamp(rate, 0);
	  RFWPI_TalonSRX.configOpenloopRamp(rate, 0);
	  RRWPI_TalonSRX.configOpenloopRamp(rate, 0);
	  LSlaveWPI_TalonSRX.configOpenloopRamp(rate, 0);
	  RSlaveWPI_TalonSRX.configOpenloopRamp(rate, 0);
  }
  
  // Return voltage and current draw for each CAN Talon.
  
  public String GetWPI_TalonSRXStatus()
  {
	  return String.format("%.1f/%.1f  %.1f/%.1f  %.1f/%.1f  %.1f/%.1f  %.1f/%.1f  %.1f/%.1f", 
			  LFWPI_TalonSRX.getMotorOutputVoltage(), LFWPI_TalonSRX.getOutputCurrent(),
			  LRWPI_TalonSRX.getMotorOutputVoltage(), LRWPI_TalonSRX.getOutputCurrent(),
			  RFWPI_TalonSRX.getMotorOutputVoltage(), RFWPI_TalonSRX.getOutputCurrent(),
			  RRWPI_TalonSRX.getMotorOutputVoltage(), RRWPI_TalonSRX.getOutputCurrent(),
			  LSlaveWPI_TalonSRX.getMotorOutputVoltage(), LSlaveWPI_TalonSRX.getOutputCurrent(),
			  RSlaveWPI_TalonSRX.getMotorOutputVoltage(), RSlaveWPI_TalonSRX.getOutputCurrent());
  }
}
