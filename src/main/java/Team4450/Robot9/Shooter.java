...
public class Shooter
{
	...
	//private final WPI_TalonSRX		pickupMotor = new WPI_TalonSRX(7);
	...
	private final ValveDA			pickupCylinder = new ValveDA(6);
	private final ValveDA			hoodCylinder = new ValveDA(4);
	...
	Shooter(Robot robot, Teleop teleop)
	{
		...
		// Handle the fact that the pickup motor is a WPI_TalonSRX on competition robot
		...
		if (robot.isComp) 
		{
			pickupMotor = new WPI_TalonSRX(7);
			robot.Initialize_WPI_TalonSRX((WPI_TalonSRX) pickupMotor);
	...
	public void dispose()
	{
		...
		if (pickupMotor != null)
		{
			if (robot.robotProperties.getProperty("RobotId").equals("comp"))
				((WPI_TalonSRX) pickupMotor).free();
			else
				((Talon) pickupMotor).close();
		}

		if (shooterPidController != null)
		{
			shooterPidController.disable();
			shooterPidController.close();
		}
		
		if (shooterMotor1 != null) shooterMotor1.close();;
		if (shooterMotor2 != null) shooterMotor2.close();;
		if (pickupCylinder != null) pickupCylinder.dispose();
		if (hoodCylinder != null) hoodCylinder.dispose();
		if (pickupSwitch != null) pickupSwitch.close();;
		if (encoder != null) encoder.close();;
		//if (turretMotor != null) turretMotor.free();
		if (turretEncoder != null) turretEncoder.close();
	}
