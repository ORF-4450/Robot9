...
class Teleop
{
	...
	private final ValveDA		tiltValve = new ValveDA(1, 0);
	private final ValveDA		climberArmsValve = new ValveDA(1, 2);
	private final ValveDA		defenseArmsValve = new ValveDA(1, 4);
	...
	void dispose()
	{
		...
		if (climbUpSwitch != null) climbUpSwitch.close();
		if (encoder != null) encoder.close();
		if (headLight != null) headLight.close();
	...
	void OperatorControl() throws Exception
	{
        ...
        if (robot.isComp) robot.SetWPI_TalonSRXBrakeMode(lpRLBControl.latchedState);
	    ...
	    public void SwitchChange(LaunchPadEvent launchPadEvent) 
	    {
	    	...
	    	switch(control.id)
	    	{
	    		...
	    		case ROCKER_LEFT_BACK:
    				if (control.latchedState)
    					robot.SetWPI_TalonSRXBrakeMode(false);	// coast
    				else
    	    			robot.SetWPI_TalonSRXBrakeMode(true);	// brake
	...
	private class UtilityStickListener implements JoyStickEventListener 
	{
	    public void ButtonDown(JoyStickEvent joyStickEvent) 
	    {
			...
			switch(button.id)
			{
				...
				case TRIGGER:
    				shooter.StartShoot(true, shooterPower);
    				break;
